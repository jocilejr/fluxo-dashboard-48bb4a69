...
export default function PaymentFlow({ open, onOpenChange, offer, themeColor, memberPhone }: Props) {
  ...
  const handleBoleto = () => setStep("boleto");
  ...
  const submitBoleto = async () => {
    ...
    try {
      ...
      if (!settings?.webhook_url) {
        toast.error("Boleto não configurado no momento");
        setBoletoLoading(false);
        return;
      }

      const payload = {
        nome: boletoName.trim(),
  ...
  const priceFormatted = offer.price
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-md rounded-2xl border-0 p-0 gap-0 overflow-hidden shadow-2xl bg-white [&>button]:hidden">
        ...
        >
          {step !== "select" && (
            <button onClick={() => setStep("select")} className="text-white/80 hover:text-white transition-colors">
        ...
        {step === "select" && (
          <div className="p-5 space-y-3">
            ...
            >
              ...
              <div>
                <p className="font-semibold text-gray-800 text-sm">Boleto Bancário</p>
        ...
        {step === "pix" && (
          <div className="p-5 space-y-4">
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              ...
              <code className="text-sm text-gray-800 font-mono bg-white px-3 py-2 rounded-lg border break-all block">
                {offer.pix_key}
              </code>
              <Button
            ...
            >
              <p className="text-sm text-gray-700 leading-relaxed">
                ✨ Ao efetuar o PIX, todo o material será <strong>liberado no seu WhatsApp</strong> automaticamente!
              </p>
            </div>
          </div>
        )}
        ...
        {step === "boleto" && boletoSent && (
          <div className="p-5 text-center space-y-3">
            ...
            <p className="text-sm text-gray-500">
              Você receberá os detalhes do boleto no seu WhatsApp em breve.
            </p>
          </div>
        )}
      </DialogContent>
