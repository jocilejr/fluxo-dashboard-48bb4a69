...
serve(async (req) => {
  ...
  try {
    ...
    // Build user_data with SHA256 hashed values
    ...
    if (first_name) {
      userData.fn = [await sha256(first_name.toLowerCase().trim())];
    }
    if (last_name) {
      userData.ln = [await sha256(last_name.toLowerCase().trim())];
    }
    ...
    for (const pixel of pixels) {
      ...
      try {
        ...
        console.log(`[CAPI] ${pixel_id}: ${response.ok ? "OK" : "ERROR"}`, JSON.stringify(result));
      } catch (err) {
        results.push({ pixel_id, status: "error", error: String(err) });
        console.error(`[CAPI] ${pixel_id} fetch error:`, err);
      }
    }
