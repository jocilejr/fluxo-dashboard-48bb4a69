...
serve(async (req) => {
  ...
  try {
    ...
    leads.slice(0, 100).forEach((lead: any, index: number) => {
      ...
      const responses: { field: string; question: string | null; aiResponse: string | null; value: string }[] = [];
      textAnswers.forEach((a: any) => {
        if (a.value && a.value.trim()) {
          responses.push({ 
            field: a.key, 
            question: a.question || null,
            aiResponse: a.aiResponse || null, // Include AI response from typebot-stats
            value: a.value.trim() 
          });
